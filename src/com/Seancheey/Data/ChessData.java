...
public class ChessData{
	...
	public static boolean putChess(int chessType,int x,int y){
		if(chessField[x][y]==0){
			...
			if(Data.Option_mode==Data.PVC&&chessType==nextChess){
				think(computer);
			}
			return true;
		}else return false;
	...
	private static void listScoreField(){
		for(int y=0;y<Data.boardsize;y++){
			for(int x=0;x<Data.boardsize;x++){
				...
				if(scoreField[x][y]>9&&scoreField[x][y]<=99){
					System.out.print(scoreField[x][y]+"  ");
				}else
				if(scoreField[x][y]>99){
					System.out.print(scoreField[x][y]+" ");
				}else{
					System.out.print("??? ");
				}
	...
	private static void addScore(int addedScore,int length,int shape, int x, int y){
		if(addedScore!=0){
			if(shape==1){
				for(int plus=0;plus<length;plus++){scoreField[x+plus][y]+=addedScore;
				}
			}else
			if(shape==2){
				for(int plus=0;plus<length;plus++){scoreField[x][y+plus]+=addedScore;
				}
			}else
			if(shape==3){
				for(int plus=0;plus<length;plus++){scoreField[x+plus][y+plus]+=addedScore;
				}
			}else
			if(shape==4){
				for(int plus=0;plus<length;plus++){scoreField[x-plus][y+plus]+=addedScore;
	...
	private static int giveScore(int chessType,int length,int[] c){
		...
		if(length==6){
			if(c[0]==0&&c[1]==r&&c[2]==r&&c[3]==r&&c[4]==r&&c[5]==0){return 432;//+OOOO+
			}else 
			if(c[0]==0&&c[1]==r&&c[2]==r&&c[3]==r&&c[4]==0&&c[5]==0){return 72;//+OOO++	
			}else
			if(c[0]==0&&c[1]==0&&c[2]==r&&c[3]==r&&c[4]==r&&c[5]==0){return 72;//++OOO+				
			}else
			if(c[0]==0&&c[1]==r&&c[2]==r&&c[3]==0&&c[4]==r&&c[5]==0){return 72;//+OO+O+				
			}else
			if(c[0]==0&&c[1]==r&&c[2]==0&&c[3]==r&&c[4]==r&&c[5]==0){return 72;//+O+OO+				
			}else
			if(c[0]==0&&c[1]==0&&c[2]==r&&c[3]==r&&c[4]==0&&c[5]==0){return 9;//++OO++				
			}else
			if(c[0]==0&&c[1]==0&&c[2]==r&&c[3]==0&&c[4]==r&&c[5]==0){return 9;//++O+O+				
			}else
			if(c[0]==0&&c[1]==r&&c[2]==0&&c[3]==r&&c[4]==0&&c[5]==0){return 9;//+O+O++				
			}else
			if(c[0]==0&&c[1]==0&&c[2]==0&&c[3]==0&&c[4]==r&&c[5]==0){return 9;//++++O+				
			}else
			if(c[0]==0&&c[1]==0&&c[2]==0&&c[3]==r&&c[4]==0&&c[5]==0){return 9;//+++O++				
			}else
			if(c[0]==0&&c[1]==0&&c[2]==r&&c[3]==0&&c[4]==0&&c[5]==0){return 9;//++O+++				
			}else
			if(c[0]==0&&c[1]==r&&c[2]==0&&c[3]==0&&c[4]==0&&c[5]==0){return 9;//+O++++				
			}
		}else
		if(length==5){
			if(c[0]==0&&c[1]==r&&c[2]==r&&c[3]==r&&c[4]==r){return 210;//+OOOO
			}else
			if(c[0]==r&&c[1]==0&&c[2]==r&&c[3]==r&&c[4]==r){return 210;//O+OOO
			}else
			if(c[0]==r&&c[1]==r&&c[2]==0&&c[3]==r&&c[4]==r){return 210;//OO+OO
			}else
			if(c[0]==r&&c[1]==r&&c[2]==r&&c[3]==0&&c[4]==r){return 210;//OOO+O
			}else
			if(c[0]==r&&c[1]==r&&c[2]==r&&c[3]==r&&c[4]==0){return 210;//OOOO+
			}else
			if(c[0]==0&&c[1]==0&&c[2]==0&&c[3]==0&&c[4]==r){return 1;//++++O
			}else
			if(c[0]==r&&c[1]==0&&c[2]==0&&c[3]==0&&c[4]==0){return 1;//O++++
			}
		}
		return 0;
	}
	...
	private static int checkWin(){
		...
		for(int x=0;x<Data.boardsize;x++){
			for(int y=0;y<Data.boardsize;y++){
				try{
				for(int ba=0;ba<5;ba++){operate[ba]=chessField[x+ba][y];}
				if(check5inrow(operate)==WHITE){return WHITE;}
				if(check5inrow(operate)==BLACK){return BLACK;}}catch(Exception e){}
				try{
				for(int ba=0;ba<5;ba++){operate[ba]=chessField[x][y+ba];}
				if(check5inrow(operate)==WHITE){return WHITE;}
				if(check5inrow(operate)==BLACK){return BLACK;}}catch(Exception e){}
				try{
				for(int ba=0;ba<5;ba++){operate[ba]=chessField[x+ba][y+ba];}
				if(check5inrow(operate)==WHITE){return WHITE;}
				if(check5inrow(operate)==BLACK){return BLACK;}}catch(Exception e){}
				try{
				for(int ba=0;ba<5;ba++){operate[ba]=chessField[x-ba][y+ba];}
				...
				for(int shape=1;shape<=4;shape++){
					...
					if(giveScore(player,5,c)==210){
						...
						for(int plus=0;plus<5;plus++){
							switch(shape){
							case 1:
								if(chessField[x+plus][y]==0){putChess(player,x+plus,y);break all;}
								break;
							case 2:
								if(chessField[x][y+plus]==0){putChess(player,x,y+plus);break all;}
								break;
							case 3:
								if(chessField[x+plus][y+plus]==0){putChess(player,x+plus,y+plus);break all;}
								break;
							case 4:
								if(chessField[x-plus][y+plus]==0){putChess(player,x-plus,y+plus);break all;}
								break;
